...
export default function DesktopNav(){
    ...
    return (
        <div className="bg-white rounded-2xl flex flex-col justify-between md:min-w-76 md:h-186 p-6">
            <div className="flex flex-col gap-12">
              <div className="flex items-center">
                <Image src='/veda-logo.svg' width={80} height={80} alt="Veda Ai" className="translate-y-3  -translate-x-5"/>
                <h1 className="text-2xl font-medium -translate-x-8">Veda AI</h1>
              </div>


            <div className="rounded-[100px] bg-linear-to-b from-[#FF7950] to-[#C0350A] flex items-center justify-center p-1 ">
            ...
            <ul className="flex flex-col gap-2">
            {
                navLinks.map((link: {title: string, icon: any, url: string},index: number) => (
                    <li key={index} className="text-[#5E5E5ECC] flex gap-2 rounded-lg cursor-pointer hover:text-[#303030] px-3 py-2 hover:bg-[#F0F0F0]" onClick={() => router.push(`/${link.url}`)}>
                        {link.icon}
                        {link.title}
                    </li>
                ))
